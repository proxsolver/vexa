...
export function AdminGuard({ children }: AdminGuardProps) {
  ...
  useEffect(() => {
    const checkSession = async () => {
      // Admin role users bypass the token check
      if (isUserRoleAdmin) {
        setIsChecking(false);
        return;
      }
    ...
    checkSession();
  }, [isAdminAuthenticated, isUserRoleAdmin]);
  ...
  // Admin role users or admin token authenticated users can access
  if (isUserRoleAdmin || isAdminAuthenticated) {
    return <>{children}</>;
  }
